...
def interpret_pose(line, do_3d):
    ...
    if len(line) == 12:
        ...
        raise ValueError('Not supported %d length' % len(line))
    return gg

def load_sequence(j, feat_suffix='', do_3d=False, noise=None, verbose=True):
    sequence_dict = {}
    ...
    if 'features' in j.keys():
        feats=np.copy(j['features']).astype(np.float32)
    else:
        feats=[]
    sequence_dict['feats'] = feats
    ...
    ###################
    for key in j.keys():
        if key!='poses' and key!='odom_poses' and key!='features':
            sequence_dict[key]=j[key]
    
    '''if 'clusters_list' in j.keys(): sequence_dict['clusters_list']=j['clusters_list']
    else: sequence_dict['cluster_list']=[]
    if 'clusters' in j.keys(): sequence_dict['clusters']=j['clusters']
    else: sequence_dict['clusters']=[]'''
